...
const REACT_ICON_COMPONENTS = path.resolve(TEMPLATE_DIR, './react-ts/components');
const REACT_JS_ICON_COMPONENTS = path.resolve(TEMPLATE_DIR, './react-js/components');
...
for (let i = 0; i < components.length; i += 1) {
  ...
  fs.writeFileSync(path.resolve(REACT_JS_ICON_COMPONENTS, name.replace('ts', 'js')), res.code);
}
